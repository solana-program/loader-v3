...
impl LoaderV3Instruction {
    ...
    pub fn pack(&self) -> Vec<u8> {
        match self {
            ...
            Self::Upgrade => vec![3],
            Self::SetAuthority => vec![4],
            Self::Close => vec![5],
            Self::ExtendProgram { additional_bytes } => {
                let mut data = vec![6];
                data.extend_from_slice(&additional_bytes.to_le_bytes());
                data
            }
            Self::SetAuthorityChecked => vec![7],
        }
    }
    ...
    pub fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        match input.split_first() {
            Some((&0, _)) => Ok(Self::InitializeBuffer),
            ...
            Some((&2, rest)) if rest.len() == 8 => {
                let max_data_len = usize::from_le_bytes(rest[..8].try_into().unwrap());
                Ok(Self::DeployWithMaxDataLen { max_data_len })
            }
            Some((&3, _)) => Ok(Self::Upgrade),
            Some((&4, _)) => Ok(Self::SetAuthority),
            Some((&5, _)) => Ok(Self::Close),
            Some((&6, rest)) if rest.len() == 4 => {
                let additional_bytes = u32::from_le_bytes(rest[..4].try_into().unwrap());
                Ok(Self::ExtendProgram { additional_bytes })
            }
            Some((&7, _)) => Ok(Self::SetAuthorityChecked),
...
pub fn initialize_buffer(source_address: &Pubkey, authority_address: &Pubkey) -> Instruction {
    ...
    let data = LoaderV3Instruction::InitializeBuffer.pack();
...
pub fn write(
    ...
    let data = LoaderV3Instruction::Write { offset, bytes }.pack();
    Instruction::new_with_bytes(crate::id(), &data, accounts)
}
...
pub fn deploy_with_max_data_len(
    ...
    let data = LoaderV3Instruction::DeployWithMaxDataLen { max_data_len }.pack();
    Instruction::new_with_bytes(crate::id(), &data, accounts)
}
...
pub fn upgrade(
    ...
    let data = LoaderV3Instruction::Upgrade.pack();
    Instruction::new_with_bytes(crate::id(), &data, accounts)
}
...
pub fn set_authority(
    ...
    let data = LoaderV3Instruction::SetAuthority.pack();
    Instruction::new_with_bytes(crate::id(), &data, accounts)
}
...
pub fn close(
    ...
    let data = LoaderV3Instruction::Close.pack();
    Instruction::new_with_bytes(crate::id(), &data, accounts)
}
...
pub fn extend_program(
    ...
    let data = LoaderV3Instruction::ExtendProgram { additional_bytes }.pack();
    Instruction::new_with_bytes(crate::id(), &data, accounts)
}
...
pub fn set_authority_checked(
    ...
    let data = LoaderV3Instruction::SetAuthorityChecked.pack();
    Instruction::new_with_bytes(crate::id(), &data, accounts)
}
...
mod tests {
    ...
    #[test]
    fn test_pack_unpack_initialize_buffer() {
        let original = LoaderV3Instruction::InitializeBuffer;
        let packed = original.pack();
        let unpacked = LoaderV3Instruction::unpack(&packed).unwrap();
        assert_eq!(original, unpacked);
    }

    #[test]
    fn test_pack_unpack_write() {
        let original = LoaderV3Instruction::Write {
            offset: 42,
            bytes: vec![1, 2, 3],
        };
        let packed = original.pack();
    ...
    #[test]
    fn test_pack_unpack_deploy_with_max_data_len() {
        let original = LoaderV3Instruction::DeployWithMaxDataLen { max_data_len: 42 };
        let packed = original.pack();
        let unpacked = LoaderV3Instruction::unpack(&packed).unwrap();
        assert_eq!(original, unpacked);
    }
    ...
    #[test]
    fn test_pack_unpack_upgrade() {
        let original = LoaderV3Instruction::Upgrade;
        let packed = original.pack();
        let unpacked = LoaderV3Instruction::unpack(&packed).unwrap();
        assert_eq!(original, unpacked);
    }
    ...
    fn test_pack_unpack_set_authority() {
        let original = LoaderV3Instruction::SetAuthority;
        let packed = original.pack();
        let unpacked = LoaderV3Instruction::unpack(&packed).unwrap();
        assert_eq!(original, unpacked);
    }
    ...
    #[test]
    fn test_pack_unpack_close() {
        let original = LoaderV3Instruction::Close;
        let packed = original.pack();
        let unpacked = LoaderV3Instruction::unpack(&packed).unwrap();
        assert_eq!(original, unpacked);
    }
    ...
    #[test]
    fn test_pack_unpack_extend_program() {
        let original = LoaderV3Instruction::ExtendProgram {
            additional_bytes: 42,
        };
        let packed = original.pack();
        let unpacked = LoaderV3Instruction::unpack(&packed).unwrap();
    ...
    #[test]
    fn test_pack_unpack_set_authority_checked() {
        let original = LoaderV3Instruction::SetAuthorityChecked;
        let packed = original.pack();
        let unpacked = LoaderV3Instruction::unpack(&packed).unwrap();
        assert_eq!(original, unpacked);
    }
